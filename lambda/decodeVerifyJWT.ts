...
interface PublicKeyMeta {
  instance: jwkToBuffer.PublicKey;
  pem: string;
}

interface PublicKeys {
  keys: jwkToBuffer.PublicKey[];
}
...
const handler = async (request: ClaimVerifyRequest): Promise<ClaimVerifyResult> => {
  ...
  try {
    ...
    result = { clientId: claim.client_id, isValid: true, userName: claim.username };
  } catch (error) {
    result = { clientId: '', error, isValid: false, userName: '' };
  }
  return result;
};
